...
## 使用docker构建draw.io 实例

```
docker run -it --rm --name drawio -p 8080:8080 -p 8443:8443 jgraph/drawio
```

这一步虽然不是“build”，但建议你先跑通，确认 embed 流程没问题，再进入“自定义构建”。官方给的 Quick Start 就是：[2](https://github.com/jgraph/docker-drawio)

然后浏览器访问（官方 README 示例带了 `offline=1`）：[2](https://github.com/jgraph/docker-drawio)

- HTTP：`http://localhost:8080/?offline=1&https=0`
- HTTPS：`https://localhost:8443/?offline=1`

> `?offline=1` 在该镜像文档里被明确描述为“安全特性：禁用云存储支持”。[2](https://github.com/jgraph/docker-drawio)

**当作 embed 端点时**，你通常给外部系统配置的 URL 会长这样：

- `http://<你的域名或IP>:8080/?embed=1&proto=json&spin=1&offline=1&https=0`
- 或 `https://<你的域名或IP>:8443/?embed=1&proto=json&spin=1&offline=1`

其中 embed mode 的关键参数与协议（`init/load/save` 消息流）官方有完整说明：
